...
(() => {
    ...
    const mostrarPaso = async (nuevoIndice) => {
        ...
        if (paso.selector) {
            objetivoActual = document.querySelector(paso.selector);
            if (objetivoActual) {
...
#suite-tour-card.visible {
    display: block;
    animation: suiteTourEnter 180ms ease-out;
}
#suite-tour-close {
    position: absolute;
    top: 10px;
...
#suite-tour-actions > div {
    display: flex;
    gap: 8px;
}
#suite-tour-actions button {
    min-height: 40px;
    padding: 8px 14px;
...
#suite-tour-actions button:disabled {
    cursor: not-allowed;
    opacity: 0.45;
}
#suite-tour-next {
